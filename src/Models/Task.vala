...
*/
namespace Notejot {
    public class Task : Object, Json.Serializable {
        ...
        public static Task from_json (Json.Node node) requires (node.get_node_type () == OBJECT) {
            return (Task) Json.gobject_deserialize (typeof (Task), node);
        }

        public static List<Task> list_from_json (Json.Node node) requires (node.get_node_type () == ARRAY) {
            var result = new List<Task> ();

            var json_array = node.get_array ();
            json_array.foreach_element ((_, __, element_node) => {
                result.append (Task.from_json (element_node));
            });
