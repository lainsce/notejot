...
*/
namespace Notejot {
    public class Note : Object, Json.Serializable {
        ...
        public static Note from_json (Json.Node node) requires (node.get_node_type () == OBJECT) {
            return (Note) Json.gobject_deserialize (typeof (Note), node);
        }

        public static List<Note> list_from_json (Json.Node node) requires (node.get_node_type () == ARRAY) {
